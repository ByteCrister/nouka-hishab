...
} from '@/constants/sand/boats.const';
import type { BoatStatus } from '@/constants/db/boats.const';
...
export const useBoatsListStore = create<BoatsListState>((set, get) => ({
    ...
    setSector: (id) =>
        set((s) => ({ filters: { ...s.filters, sectorId: id, page: 1 } })),
    setSort: (by, order) =>
        set((s) => ({
            filters: {
                ...s.filters,
