...
import { MapPin, Clock, Package, TrendingUp, Ship, Layers } from 'lucide-react';
...
export function SandTripCard({ trip }: Props) {
  ...
  return (
    <Link href={`/trips/sand/${trip.publicId}`}>
      <Card className="group overflow-hidden border-border/50 bg-card hover:shadow-lg hover:shadow-primary/5 hover:border-primary/20 transition-all duration-300 cursor-pointer h-full flex flex-col justify-between">
        <CardContent className="p-5 flex flex-col gap-4 flex-1">
          ...
          <div className="flex items-center justify-between gap-2">
            ...
            <div className="flex items-center gap-2">
              <Badge variant="secondary" className="text-[10px] uppercase tracking-wider bg-amber-500/10 text-amber-600 dark:text-amber-400 hover:bg-amber-500/20 border-amber-500/20">
                <Layers className="w-3 h-3 mr-1" /> Sand
              </Badge>
