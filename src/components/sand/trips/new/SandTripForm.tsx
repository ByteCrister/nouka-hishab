...
import { SandTripDetail, CreateSandTripPayload } from '@/types/sand/trips.types';
...
export function SandTripForm({ initialData }: { initialData?: SandTripDetail }) {
  ...
  const { mutateAsync: updateTrip, isPending: isUpdating } = useUpdateSandTrip(() => {
    router.push(`/sand/trips/${initialData?.publicId}`);
  });
        ...
        <div className="relative z-10">
          ...
          <div className="flex items-center justify-between mb-8">
            ...
            <Button variant="ghost" asChild className="h-9 px-4 hidden sm:flex">
              <Link href={isEdit ? `/sand/trips/${initialData?.publicId}` : "/sand/trips"}>
          ...
          <form onSubmit={handleSubmit} className="space-y-8">
            ...
            <div className="pt-4 border-t border-border flex items-center justify-between">
              <Button type="button" variant="ghost" onClick={() => router.push(isEdit ? `/sand/trips/${initialData?.publicId}` : '/sand/trips')} disabled={isPending} className="h-11 px-6 rounded-xl sm:hidden">
                {t('cancel')}
              </Button>
              <Button
                type="submit"
